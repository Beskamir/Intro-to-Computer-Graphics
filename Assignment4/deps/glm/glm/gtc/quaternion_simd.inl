...
*/
	...
	struct compute_dot<tquat, float, P, true>
	{
		...
		{
			return _mm_cvtss_f32(glm_vec1_dot(x.shaderData, y.shaderData));
		}
	};
	...
	struct compute_quat_add<float, P, true>
	{
		...
		{
			tquat<float, P> Result(uninitialize);
			Result.shaderData = _mm_add_ps(q.shaderData, p.shaderData);
			return Result;
		}
	};
...
#	if GLM_ARCH & GLM_ARCH_AVX_BIT
	...
	struct compute_quat_add<double, P, true>
	{
		static tquat<double, P> call(tquat<double, P> const & a, tquat<double, P> const & b)
		{
			...
			Result.shaderData = _mm256_add_pd(a.shaderData, b.shaderData);
			return Result;
		}
	};
#	endif
	...
	struct compute_quat_sub<float, P, true>
	{
		static tquat<float, P> call(tquat<float, P> const& q, tquat<float, P> const& p)
		{
			...
			Result.shaderData = _mm_sub_ps(q.shaderData, p.shaderData);
			return Result;
		}
	};
...
#	if GLM_ARCH & GLM_ARCH_AVX_BIT
	...
	struct compute_quat_sub<double, P, true>
	{
		...
		{
			tquat<double, P> Result(uninitialize);
			Result.shaderData = _mm256_sub_pd(a.shaderData, b.shaderData);
			return Result;
		}
	};
#	endif
	...
	struct compute_quat_mul_scalar<float, P, true>
	{
		...
		{
			tvec4<float, P> Result(uninitialize);
			Result.shaderData = _mm_mul_ps(q.shaderData, _mm_set_ps1(s));
			return Result;
		}
	};
...
#	if GLM_ARCH & GLM_ARCH_AVX_BIT
	...
	struct compute_quat_mul_scalar<double, P, true>
	{
		...
		{
			tquat<double, P> Result(uninitialize);
			Result.shaderData = _mm256_mul_pd(q.shaderData, _mm_set_ps1(s));
			return Result;
		}
	};
#	endif
	...
	struct compute_quat_div_scalar<float, P, true>
	{
		...
		{
			tvec4<float, P> Result(uninitialize);
			Result.shaderData = _mm_div_ps(q.shaderData, _mm_set_ps1(s));
			return Result;
		}
	};
...
#	if GLM_ARCH & GLM_ARCH_AVX_BIT
	...
	struct compute_quat_div_scalar<double, P, true>
	{
		...
		{
			tquat<double, P> Result(uninitialize);
			Result.shaderData = _mm256_div_pd(q.shaderData, _mm_set_ps1(s));
			return Result;
		}
	};
#	endif
	...
	struct compute_quat_mul_vec4<float, P, true>
	{
		...
		{
			__m128 const q_wwww = _mm_shuffle_ps(q.shaderData, q.shaderData, _MM_SHUFFLE(3, 3, 3, 3));
			__m128 const q_swp0 = _mm_shuffle_ps(q.shaderData, q.shaderData, _MM_SHUFFLE(3, 0, 2, 1));
			__m128 const q_swp1 = _mm_shuffle_ps(q.shaderData, q.shaderData, _MM_SHUFFLE(3, 1, 0, 2));
			__m128 const v_swp0 = _mm_shuffle_ps(v.shaderData, v.shaderData, _MM_SHUFFLE(3, 0, 2, 1));
			__m128 const v_swp1 = _mm_shuffle_ps(v.shaderData, v.shaderData, _MM_SHUFFLE(3, 1, 0, 2));
			...
			tvec4<float, P> Result(uninitialize);
			Result.shaderData = _mm_add_ps(v.Data, _mm_add_ps(uv, uuv));
			return Result;
		}
	};
}//namespace detail
